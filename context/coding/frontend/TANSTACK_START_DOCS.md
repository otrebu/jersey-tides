...
> [!NOTE]
> We use `pnpm` in all of these examples, but you can use your package manager of choice instead.
...
```shell
pnpm add -D typescript @types/react @types/react-dom @types/node vite-tsconfig-paths
```
...
You can now run `pnpm dev` to start your server and navigate to `http://localhost:3000` to see your route in action.
